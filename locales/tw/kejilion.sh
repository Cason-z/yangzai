...
bbrv3() {
		  ...
		  xanmod_add_repo() {
				...
				# 相容官方已移除的老系統代號（回退使用 releases 嘗試舊包庫）
				...
				# 官方已徹底移除對 jammy, focal, bullseye 等舊系統的 apt 支持
				if echo "jammy focal bullseye buster" | grep -qw "$os_codename" || [ "$os_codename" = "releases" ]; then
					echo -e "${gl_hong}XanMod 官方已停止對目前系統($os_codename)的 APT 來源支持，請升級至 Debian12 / Ubuntu24 或更高版本。${gl_bai}"
					return 1
				fi

				if [ -z "$os_codename" ]; then
					echo "無法取得系統代號，無法設定XanMod來源"
					return 1
				fi
		  ...
		  xanmod_detect_package() {
				...
				echo "軟體來源中找不到適合此CPU的XanMod核心包" >&2
				return 1
		  }
		  ...
		  xanmod_install_or_update() {
				...
				check_swap
				xanmod_add_repo || {
					echo "XanMod官方倉庫配置失敗，請稍後重試"
					return 1
				}

				package=$(xanmod_detect_package) || {
					echo "無法辨識目前CPU或找不到匹配核心包，已取消安裝"
					return 1
				}
				...
				if [ "$action" = "update" ]; then
					apt install -y --only-upgrade "$package" || apt install -y "$package" || {
						echo "XanMod核心更新失敗，請檢查軟體來源或稍後重試"
						return 1
					}
				else
					apt install -y "$package" || {
						echo "XanMod核心安裝失敗，請檢查軟體來源或稍後重試"
						return 1
					}
				fi
...
clamav() {
		  ...
		  while true; do
				...
				echo "------------------------"
				echo -e "${gl_lv}1. 全盤掃描${gl_bai}             ${gl_huang}2. 重要目錄掃描${gl_bai}            ${gl_kjlan} 3. 自定义目录扫描 ${gl_bai}"
				echo "------------------------"
				echo "0. 返回上一級選單"
				echo "------------------------"
				read -e -p "請輸入你的選擇:" sub_choice
...
EOF
	...
	openclaw_memory_render_status() {
		...
		if [ -z "$json_output" ]; then
			echo "取得記憶狀態失敗（openclaw memory status --json 無輸出）"
			return 1
		fi
...
for entry in data:
    ...
    print("底層方案: %s" % backend)
    files = s.get("files", 0)
    chunks = s.get("chunks", 0)
    print("已收錄: %s 檔案 / %s 區塊" % (files, chunks))
    dirty = s.get("dirty")
    dirty_str = "是" if dirty else "否"
    print("待刷新: %s" % dirty_str)
    vec = s.get("vector", {})
    if isinstance(vec, dict) and vec.get("enabled"):
        vec_str = "就緒" if vec.get("available") else "已啟用(不可用)"
    else:
        vec_str = "未啟用"
    print("向量庫: %s" % vec_str)
...
try:
    ...
    print("攔截策略 (Security):" + str(sec))
    print("提示策略 (Ask):" + str(ask))
    print("無UI兜底 (AskFallback):" + str(fb))
    print("自動放行技能 (autoAllowSkills):" + ("on" if auto else "off"))
    exists = d.get("exists", True)
    if not exists:
        print("(審核文件不存在，使用系統內建安全兜底)")
except Exception as e:
    print("(解析失敗:" + str(e) + ")")
' "$approvals_json"
			else
				echo "(openclaw approvals get --json 無輸出)"
			fi
...
' "$allowlist_json"
			...
			case "$al_choice" in
				1)
					read -e -p "輸入要放行的命令路徑 (支援 glob，如 /usr/bin/git):" pattern
					[ -z "$pattern" ] && { echo "不能為空"; break_end; continue; }
					read -e -p "指定智能體ID (留空=所有智能體 *):" agent_id
					agent_id="${agent_id:-*}"
					openclaw approvals allowlist add --agent "$agent_id" "$pattern"
					break_end
					;;
				2)
					read -e -p "輸入要移除的命令路徑:" pattern
					[ -z "$pattern" ] && { echo "不能為空"; break_end; continue; }
...
print("路由綁定數=%s" % len(bindings))
print("✅ 多智能體健康檢查完成")
' "$(openclaw_multiagent_agents_json)" "$(openclaw_multiagent_bindings_json)"
		echo ""
		echo "運行安全審計..."
		openclaw security audit 2>/dev/null || echo "⚠️ 安全審計命令不可用"
	}
	...
	openclaw_multiagent_set_identity() {
		...
		read -e -p "是否從 IDENTITY.md 讀取？ (y/n):" from_id
		if [ "$from_id" = "y" ]; then
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}第三方應用程式列表"
  	  echo -e "${gl_kjlan}想要讓你的應用程式出現在這裡？查看開發者指南:${gl_huang}洋仔${gl_bai}"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}k指令進階用法${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}解除安裝洋仔腳本"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}返回主選單"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "請輸入你的選擇:" sub_choice
...
EOF
		  ...
		  61)
			clear
			send_stats "留言板"
			echo "造訪洋仔官方留言板，您對腳本有任何想法歡迎留言交流！"
			echo "洋仔"
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}安裝洋仔腳本${gl_kjlan}12. ${gl_bai}更新系統${gl_kjlan}13. ${gl_bai}清理系統"
...
echo -e "${gl_bai}網址: 洋仔${gl_bai}"
echo "------------------------"
echo ""
echo -e "網域優惠"
echo "------------------------"
echo -e "${gl_lan}GNAME 8.8刀首年COM域名 6.68刀首年CC域名${gl_bai}"
echo -e "${gl_bai}網址: https://www.gname.com/register${gl_bai}"
echo "------------------------"
echo ""
echo -e "洋仔週邊"
echo "------------------------"
echo -e "${gl_kjlan}B站:${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}油管:${gl_bai}洋仔${gl_bai}"
echo -e "${gl_kjlan}官網:${gl_bai}洋仔              ${gl_kjlan}導航:${gl_bai}洋仔${gl_bai}"
echo -e "${gl_kjlan}部落格:${gl_bai}洋仔         ${gl_kjlan}軟體中心:${gl_bai}洋仔${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}腳本官網:${gl_bai}洋仔            ${gl_kjlan}GitHub地址:${gl_bai}洋仔${gl_bai}"
echo "------------------------"
echo ""
}
