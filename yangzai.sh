...
UserLicenseAgreement() {
	clear
	echo -e "${gl_kjlan}欢迎使用洋仔脚本工具箱${gl_bai}"
...
auto_root_ssh_detect_system() {
	...
	echo "$os_id|$os_name|$os_version"
}


auto_root_ssh_install_package() {
	echo "1/6 正在识别包管理器并安装 OpenSSH Server..."
	...
	if command -v apt-get >/dev/null 2>&1; then
		...
		echo "未识别到受支持的包管理器，无法自动安装 OpenSSH。"
		return 1
	fi
}


auto_root_ssh_service_name() {
...
auto_root_ssh_enable_service() {
	local svc="$1"

	echo "4/6 正在设置 SSH 服务开机自启..."
	if command -v rc-update >/dev/null 2>&1; then
		rc-update add "$svc" default >/dev/null 2>&1 || rc-update add sshd default >/dev/null 2>&1 || true
...
auto_root_ssh_restart_service() {
	local svc="$1"

	echo "5/6 正在启动或重启 SSH 服务..."
	if [ -x /bin/systemctl ] || [ -x /usr/bin/systemctl ]; then
		systemctl restart "$svc" >/dev/null 2>&1 || systemctl restart ssh >/dev/null 2>&1 || systemctl restart sshd >/dev/null 2>&1
	elif command -v service >/dev/null 2>&1; then
		service "$svc" restart >/dev/null 2>&1 || service ssh restart >/dev/null 2>&1 || service sshd restart >/dev/null 2>&1
	elif [ -x "/etc/init.d/$svc" ]; then
		"/etc/init.d/$svc" restart >/dev/null 2>&1
	else
		echo "未找到可用的服务管理工具，请手动重启 SSH 服务。"
		return 1
	fi
}


auto_root_ssh_set_option() {
	local file="$1"
	local key="$2"
	local value="$3"

	if grep -Eq "^[[:space:]]*#?[[:space:]]*$key[[:space:]]+" "$file"; then
		sed -i "s|^[[:space:]]*#*[[:space:]]*$key[[:space:]].*|$key $value|g" "$file"
	else
		printf '%s %s\n' "$key" "$value" >> "$file"
	fi
}
...
auto_root_ssh_configure() {
	local sshd_config="/etc/ssh/sshd_config"
	local backup_file

	echo "2/6 正在检查 SSH 配置文件..."
	mkdir -p /etc/ssh
	...
	echo "3/6 正在开启 root SSH 登录和密码登录..."
	auto_root_ssh_set_option "$sshd_config" "PermitRootLogin" "yes"
	auto_root_ssh_set_option "$sshd_config" "PasswordAuthentication" "yes"
	auto_root_ssh_set_option "$sshd_config" "PubkeyAuthentication" "yes"
	auto_root_ssh_set_option "$sshd_config" "UsePAM" "yes"
	auto_root_ssh_set_option "$sshd_config" "ChallengeResponseAuthentication" "no"
	auto_root_ssh_set_option "$sshd_config" "KbdInteractiveAuthentication" "no"

	if [ -d /etc/ssh/sshd_config.d ]; then
		local override_backup="/etc/ssh/sshd_config.d.bak.$(date +%Y%m%d-%H%M%S)"
		mkdir -p "$override_backup"
		find /etc/ssh/sshd_config.d -type f -name '*.conf' -print0 2>/dev/null | while IFS= read -r -d '' conf_file; do
			cp -a "$conf_file" "$override_backup/" 2>/dev/null || true
			sed -i -e 's/^[[:space:]]*#*[[:space:]]*PermitRootLogin[[:space:]].*/PermitRootLogin yes/g' \
			       -e 's/^[[:space:]]*#*[[:space:]]*PasswordAuthentication[[:space:]].*/PasswordAuthentication yes/g' \
			       -e 's/^[[:space:]]*#*[[:space:]]*PubkeyAuthentication[[:space:]].*/PubkeyAuthentication yes/g' "$conf_file"
		done
		echo "已处理 /etc/ssh/sshd_config.d 中可能覆盖主配置的文件。"
	fi

	if command -v sshd >/dev/null 2>&1; then
		sshd -t || {
			echo "SSH 配置检测失败，正在恢复备份。"
...
auto_root_ssh_print_status() {
	local svc="$1"
	local sshd_config="/etc/ssh/sshd_config"
	local port permit_root password_auth pubkey_auth

	port=$(awk '/^[[:space:]]*Port[[:space:]]+/ {print $2; found=1} END {if (!found) print "22"}' "$sshd_config" 2>/dev/null)
	permit_root=$(awk 'tolower($1)=="permitrootlogin" {v=$2} END {print v ? v : "未设置"}' "$sshd_config" 2>/dev/null)
	password_auth=$(awk 'tolower($1)=="passwordauthentication" {v=$2} END {print v ? v : "未设置"}' "$sshd_config" 2>/dev/null)
	pubkey_auth=$(awk 'tolower($1)=="pubkeyauthentication" {v=$2} END {print v ? v : "未设置"}' "$sshd_config" 2>/dev/null)
	...
	echo "密码登录: $password_auth"
	echo "密钥登录: $pubkey_auth"
	if [ -x /bin/systemctl ] || [ -x /usr/bin/systemctl ]; then
		systemctl is-active "$svc" >/dev/null 2>&1 && echo "服务状态: 运行中" || echo "服务状态: 未确认，请手动检查"
	else
		echo "服务状态: 已尝试启动，请用 service $svc status 检查"
	fi
	echo "----------------------------------------"
	echo "提示: 如果 root 还没有密码，请先执行 passwd root 设置密码。"
	echo "提示: 开启 root 密码登录会降低安全性，建议设置强密码并改 SSH 端口。"
}


auto_root_ssh_setup() {
	root_use
	send_stats "一键安装SSH并开启ROOT登录"

	local sys_info os_id os_name os_version svc
	sys_info=$(auto_root_ssh_detect_system)
	os_id=$(echo "$sys_info" | cut -d '|' -f 1)
	os_name=$(echo "$sys_info" | cut -d '|' -f 2)
	os_version=$(echo "$sys_info" | cut -d '|' -f 3)

	clear
	echo "一键安装 SSH 并开启 ROOT 登录"
	echo "----------------------------------------"
	echo "识别到系统: $os_name"
	echo "系统 ID: $os_id"
	[ -n "$os_version" ] && echo "系统版本: $os_version"
	echo "将执行以下操作:"
	echo "1. 自动识别系统和包管理器"
	echo "2. 安装 OpenSSH Server"
	echo "3. 备份 /etc/ssh/sshd_config"
	echo "4. 开启 PermitRootLogin yes"
	echo "5. 开启 PasswordAuthentication yes"
	echo "6. 启用并重启 SSH 服务"
	echo "----------------------------------------"
	read -e -p "确认继续吗？(Y/N): " choice
	case "$choice" in
		[Yy])
			auto_root_ssh_install_package || return 1
			auto_root_ssh_configure || return 1
			svc=$(auto_root_ssh_service_name)
			auto_root_ssh_enable_service "$svc"
			auto_root_ssh_restart_service "$svc" || return 1
			auto_root_ssh_print_status "$svc"
			;;
		*)
			echo "已取消操作。"
			;;
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}第三方应用列表"
  	  echo -e "${gl_kjlan}想要让你的应用出现在这里？查看开发者指南: ${gl_huang}洋仔${gl_bai}"
...
linux_Settings() {
  ...
  echo -e "${gl_kjlan}43.  ${gl_bai}一键安装SSH并开启ROOT登录"
	  echo -e "${gl_kjlan}------------------------"
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}k命令高级用法 ${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}卸载洋仔脚本"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}返回主菜单"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
...
EOF
		  ...
		  61)
			clear
			send_stats "留言板"
			echo "访问洋仔官方留言板，您对脚本有任何想法欢迎留言交流！"
			echo "洋仔"
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}安装洋仔脚本         ${gl_kjlan}12. ${gl_bai}更新系统              ${gl_kjlan}13. ${gl_bai}清理系统"
...
echo -e "${gl_bai}网址: 洋仔${gl_bai}"
echo "------------------------"
echo ""
echo -e "域名优惠"
echo "------------------------"
echo -e "${gl_lan}GNAME 8.8刀首年COM域名 6.68刀首年CC域名${gl_bai}"
echo -e "${gl_bai}网址: https://www.gname.com/register${gl_bai}"
echo "------------------------"
echo ""
echo -e "洋仔周边"
echo "------------------------"
echo -e "${gl_kjlan}B站: ${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}油管: ${gl_bai}洋仔${gl_bai}"
echo -e "${gl_kjlan}官网: ${gl_bai}洋仔              ${gl_kjlan}导航: ${gl_bai}洋仔${gl_bai}"
echo -e "${gl_kjlan}博客: ${gl_bai}洋仔         ${gl_kjlan}软件中心: ${gl_bai}洋仔${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}脚本官网: ${gl_bai}洋仔            ${gl_kjlan}GitHub地址: ${gl_bai}洋仔${gl_bai}"
echo "------------------------"
echo ""
}
